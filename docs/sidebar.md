...
The `collapsed` and `closed` properties can also be assigned from JavaScript. An overlay sidebar remains over the page until its collapse animation finishes, then its compact rail returns to the layout. The sidebar emits `collapsechange`, `closedchange`, and `themechange` events for state changes.
