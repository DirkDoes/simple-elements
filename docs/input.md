...
```html
<se-input label="Email" name="email" type="email"></se-input>
...
Attributes: `label`, `name`, `type`, `value`, `placeholder`, `icon`, `hint`, `error`, `disabled`, `fixed`, and `autosize`. Use `fixed` for a non-resizable textarea and `autosize` to start at normal input height and grow with its content without a resize handle. Read or set the element's `value` property.
