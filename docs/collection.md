...
Use `dividers="1,4"` for vertical lines after columns 1 and 4 (one-based). Add `divider-style="dashed"` for dashed lines; the default is solid. Omit it for no dividers. It applies to row cells; headers keep the spacing without the stroke; the final column never gets an extra line. For hierarchies use `type="table"` and row `level` values. `type="tree"` retains the existing nested tree-item API.
