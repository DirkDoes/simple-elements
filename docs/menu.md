...
Use `icon-only` to hide the visible trigger label, retaining `label` as its accessible name. Menus use the browser top layer to escape table and modal clipping. Escape or an outside click dismisses them; scrolling their surrounding container closes them.
