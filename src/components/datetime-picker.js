...
class SeDatetimePicker extends HTMLElement {
  ...
  connectSingle() {
    ...
    this.querySelector('[data-month-select]').addEventListener('change', (event) => { this._view = new Date(this._view.getFullYear(), Number(event.target.value), 1); this.renderSingle(); });
