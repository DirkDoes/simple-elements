...
class SeDatePicker extends HTMLElement {
  connectedCallback() {
    ...
    this.querySelector('[data-month-select]').addEventListener('change', (event) => { this._view = new Date(this._view.getFullYear(), Number(event.target.value), 1); this.renderCalendar(); });
  ...
  renderCalendar() {
    ...
    this.querySelector('[data-month-select]').value = String(this._view.getMonth());
